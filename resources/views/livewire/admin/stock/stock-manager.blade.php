                ...
                <div class="px-5 py-4 border-t border-gray-200 dark:border-white/[0.05] sm:px-6">
                    <div class="flex items-center justify-between">
                        <p class="text-theme-sm text-gray-500 dark:text-gray-400">
                            Zobrazeno <span class="font-medium text-gray-800 dark:text-white/90">{{ $movements->firstItem() }}</span>
                            až <span class="font-medium text-gray-800 dark:text-white/90">{{ $movements->lastItem() }}</span>
                            z <span class="font-medium text-gray-800 dark:text-white/90">{{ $movements->total() }}</span>
                        </p>
                        <div class="flex items-center gap-2">
                            ...
                            @else
                                <button wire:click="previousPage('history')" class="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-2 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200">
                                    <svg width="16" height="16" viewBox="0 0 20 20" fill="none"><path fill-rule="evenodd" clip-rule="evenodd" d="M2.58301 9.99868C2.58272 10.1909 2.65588 10.3833 2.80249 10.53L7.79915 15.5301C8.09194 15.8231 8.56682 15.8233 8.85981 15.5305C9.15281 15.2377 9.15297 14.7629 8.86018 14.4699L5.14009 10.7472L16.6675 10.7472C17.0817 10.7472 17.4175 10.4114 17.4175 9.99715C17.4175 9.58294 17.0817 9.24715 16.6675 9.24715L5.14554 9.24715L8.86017 5.53016C9.15297 5.23717 9.15282 4.7623 8.85983 4.4695C8.56684 4.1767 8.09197 4.17685 7.79917 4.46984L2.84167 9.43049C2.68321 9.568 2.58301 9.77087 2.58301 9.99715C2.58301 9.99766 2.58301 9.99817 2.58301 9.99868Z" fill="currentColor"/></svg>
                                    <span class="hidden sm:inline">Předchozí</span>
                                </button>
                            @endif
                            <div class="hidden items-center gap-0.5 sm:flex">
                                @foreach($movements->getUrlRange(1, $movements->lastPage()) as $page => $url)
                                    ...
                                    @else
                                        <button wire:click="gotoPage({{ $page }}, 'history')" class="flex h-9 w-9 items-center justify-center rounded-lg text-theme-sm font-medium text-gray-700 hover:bg-brand-500/[0.08] hover:text-brand-500 dark:text-gray-400 dark:hover:text-brand-500">{{ $page }}</button>
                                    @endif
                                @endforeach
                            </div>
                            <span class="block text-sm font-medium text-gray-700 dark:text-gray-400 sm:hidden">{{ $movements->currentPage() }} / {{ $movements->lastPage() }}</span>
                            @if($movements->hasMorePages())
                                <button wire:click="nextPage('history')" class="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-2 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200">
